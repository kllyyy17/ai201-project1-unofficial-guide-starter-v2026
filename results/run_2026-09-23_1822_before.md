...
```
According to admin_graduation_requirements.txt, 120 credit hours are required for graduation.
```
...
```
Based on the documents provided, 120 credit hours are required for graduation (*admin_graduation_requirements.txt*).
```
...
```
Students should expect to spend 8 to 10 hours a week outside class for CS 210 (Source: `course_cs_210_workload.txt` and `course_cs_210.txt`).
```
...
```
At Fenwick Court, laundry costs $2.00 to wash and $1.75 to dry. This information comes from housing_fenwick_court_laundry.txt.
```
